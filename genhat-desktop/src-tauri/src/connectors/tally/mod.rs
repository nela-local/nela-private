...
pub use parse::{
    ...
    parse_outstanding_from_ledgers, resolve_company_name, DaybookLine, LedgerRow,
    OutstandingBucket, OutstandingSummary,
};
pub use xml::{
...
pub async fn list_ledgers(
    ...
    let cfg = cfg_or_err()?;
    let cap = max_rows.unwrap_or(200).clamp(1, 200);
    let group = group
        .map(|s| s.trim().to_string())
        .filter(|s| !s.is_empty());
...
pub async fn trial_balance(
    ...
    match tally_http_post(&cfg.host, cfg.port, &xml).await {
        Ok(body) => {
            let (rows, truncated) = parse_ledgers(&body, cap);
...
pub async fn daybook(
    ...
    let cfg = cfg_or_err()?;
    let cap = max_rows.unwrap_or(100).clamp(1, 100);
    let from = from_date.filter(|s| !s.trim().is_empty());
    let to = to_date.filter(|s| !s.trim().is_empty());
...
pub async fn outstanding(max_rows: Option<usize>) -> Result<TallyOutstandingResult, String> {
    let cfg = cfg_or_err()?;
    let cap = max_rows.unwrap_or(50).clamp(1, 100);
