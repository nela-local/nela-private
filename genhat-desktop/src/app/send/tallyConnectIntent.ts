...
export function looksLikeTallyRequest(text: string): boolean {
  ...
  if (
    /\b(trial\s*balance|day\s*book|sundry\s+debtors|sundry\s+creditors|receivables?|payables?|outstanding\s+(?:invoices?|bills?|balances?)|cash\s*(?:&|and)?\s*bank|sales\s+trend)\b/.test(
      t
    ) &&
  ...
  if (
    ...
    /\b(tally|day\s*book|trial\s*balance|outstanding|ledger|sales|cash)\b/.test(t)
  ) {
    return true;
  }
  if (
    /\b(accounting\s+dashboard|finance\s+dashboard|receivables?\s+dashboard|payables?\s+dashboard|sales\s+dashboard|cash\s*(?:&|and)?\s*bank\s+dashboard)\b/.test(
      t
    )
  ) {
    return true;
